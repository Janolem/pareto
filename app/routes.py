...
def pareto2():
    ...
    constraints = Constraint.query.join(Variation.variation_has_constraints).filter(
        Variation.variation_has_constraints.any(id=Variation.id))
    string_mathematical_formulation = "mathematical formulation not yet working. Has something to with unknown objective.symbols column."
    ...
    return render_template(
        ...
        output_data = {"problem_description": problem_description, "objectives": [row.name for row in objectives], "constraints": [row.name for row in constraints], "mathematical_formulation": string_mathematical_formulation}
    )
